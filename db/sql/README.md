...
- `008_add_access_role_to_users.sql` also applies baseline role assignment (`caas_admin` -> `admin`, `test` -> `user`).
...
- Admin dashboard queries analytics views `caas_admin_vw_*` from `007_create_admin_analytics_views.sql`.
